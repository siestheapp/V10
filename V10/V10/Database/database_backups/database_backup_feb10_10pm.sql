...
SELECT pg_catalog.setval('public.scan_history_id_seq', 29, true);
...
-- Update NULL user_ids in user_measurements
UPDATE user_measurements 
SET user_id = 18 
WHERE user_id IS NULL;

-- Verify the update
SELECT DISTINCT user_id FROM user_measurements;

-- First, update any references to the duplicate IDs
UPDATE user_measurements 
SET measurement_type_id = 2 
WHERE measurement_type_id = 6;

UPDATE user_measurements 
SET measurement_type_id = 3 
WHERE measurement_type_id = 7;

-- Then delete the duplicate measurement types
...
-- First, add new measurement types for Neck and Waist
INSERT INTO measurement_types (name, display_name, measurement_type, unit) 
VALUES 
('neck_circumference', 'Neck', 'circumference', 'inches'),
('waist_circumference', 'Waist', 'circumference', 'inches');

-- Get the IDs of our new measurement types
WITH new_types AS (
    SELECT id, name 
    FROM measurement_types 
    WHERE name IN ('neck_circumference', 'waist_circumference')
)
-- Update NULL measurement_type_ids based on the original name
UPDATE user_measurements um
SET measurement_type_id = (
    CASE 
        WHEN LOWER(um.original_name) LIKE '%neck%' 
          OR LOWER(um.original_name) LIKE '%collar%' 
        THEN (SELECT id FROM new_types WHERE name = 'neck_circumference')
        
        WHEN LOWER(um.original_name) LIKE '%waist%' 
        THEN (SELECT id FROM new_types WHERE name = 'waist_circumference')
    END
)
WHERE um.measurement_type_id IS NULL
AND (
    LOWER(um.original_name) LIKE '%neck%'
    OR LOWER(um.original_name) LIKE '%collar%'
    OR LOWER(um.original_name) LIKE '%waist%'
);

-- Verify the updates
SELECT mt.name as type_name, 
       um.original_name, 
       COUNT(*) as count
FROM user_measurements um
JOIN measurement_types mt ON um.measurement_type_id = mt.id
GROUP BY mt.name, um.original_name
ORDER BY mt.name;

-- Update neck measurements
UPDATE user_measurements 
SET measurement_type_id = 8
WHERE measurement_type_id IS NULL 
AND LOWER(original_name) IN ('neck', 'neck circumference', 'collar size');

-- Update waist measurements
UPDATE user_measurements 
SET measurement_type_id = 9
WHERE measurement_type_id IS NULL 
AND LOWER(original_name) = 'waist';

-- Verify the updates
SELECT id, original_name, measurement_type_id, value
FROM user_measurements 
WHERE original_name IN ('Neck', 'Neck circumference', 'Collar size', 'Waist')
ORDER BY id;
