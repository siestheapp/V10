...
CREATE FUNCTION public.sync_fit_feedback() RETURNS trigger
    ...
    AS $$
BEGIN
    -- Convert invalid values before updating
    UPDATE user_garments
    SET fit_feedback = CASE 
        WHEN NEW.overall_fit = 'Good Fit' THEN 'Perfect Fit'  -- Convert invalid value
...
--
-- Name: fit_zones; Type: TABLE; Schema: public; Owner: seandavey
--
...
CREATE TABLE public.fit_zones (
    ...
    CONSTRAINT fit_zones_fit_type_check CHECK ((fit_type = ANY (ARRAY['Tight'::text, 'Perfect'::text, 'Relaxed'::text, 'Oversized'::text]))),
    CONSTRAINT fit_zones_garment_category_check CHECK ((garment_category = ANY (ARRAY['Tops'::text, 'Jackets'::text, 'Pants'::text])))
);


ALTER TABLE public.fit_zones OWNER TO seandavey;

--
-- Name: fit_zones_id_seq; Type: SEQUENCE; Schema: public; Owner: seandavey
--
...
ALTER TABLE public.fit_zones_id_seq OWNER TO seandavey;

--
-- Name: fit_zones_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: seandavey
--
...
--
-- Name: fit_zones id; Type: DEFAULT; Schema: public; Owner: seandavey
--
...
9	user_fit_feedback	Tracks user feedback on fit (e.g., "Too Tight", "Perfect Fit", "Too Loose").	["id", "user_id", "garment_id", "overall_fit", "chest_fit", "sleeve_fit", "neck_fit", "waist_fit"]
...
11	user_garments	Stores garments a user has scanned or added for fit tracking.	["id", "user_id", "brand_id", "created_at", "size_label", "chest_range", "fit_feedback", "category"]
...
--
-- Data for Name: fit_zones; Type: TABLE DATA; Schema: public; Owner: seandavey
--

COPY public.fit_zones (id, user_id, fit_type, chest_min, chest_max, sleeve_min, sleeve_max, waist_min, waist_max, garment_category) FROM stdin;
\.
...
COPY public.user_fit_feedback (id, user_id, garment_id, overall_fit, chest_fit, sleeve_fit, neck_fit, waist_fit) FROM stdin;
3	1	1	Good Fit	Good Fit	\N	\N	\N
4	1	2	Good Fit	Good Fit	\N	\N	\N
6	1	3	Loose but I Like It	Loose but I Like It	Loose but I Like It	\N	Loose but I Like It
\.
...
1	1	1	Tops	M	39-40	Good Fit	2025-02-17 23:52:16.031871	true
2	1	1	Tops	M	39-40	Good Fit	2025-02-17 23:58:00.491465	true
4	1	1	Tops	L	42-44	Too Tight	2025-02-18 00:37:42.305611	true
5	1	1	Tops	XL	45-47	Tight but I Like It	2025-02-18 00:37:42.305611	true
6	1	1	Tops	M	38-40	Too Loose	2025-02-18 00:37:42.305611	true
3	1	2	Tops	XL	47	Loose but I Like It	2025-02-18 00:22:32.857724	true
\.
...
--
-- Name: fit_zones_id_seq; Type: SEQUENCE SET; Schema: public; Owner: seandavey
--

SELECT pg_catalog.setval('public.fit_zones_id_seq', 1, false);
...
SELECT pg_catalog.setval('public.user_fit_zones_id_seq', 1, true);
...
--
-- Name: fit_zones fit_zones_pkey; Type: CONSTRAINT; Schema: public; Owner: seandavey
--
