    ...
    if request.method == 'POST':
        try:
            ...
            def find_size_guide(cursor, brand_id, category_id, gender, fit_type, subcategory_id):
                result = None
                # First try subcategory-specific guide
                if subcategory_id:
                    cursor.execute("""
                ...
                # If no subcategory-specific guide found, try category-level guide
                if not result:
                    cursor.execute("""
                        SELECT id FROM size_guides 
                        WHERE brand_id = %s AND category_id = %s AND gender = %s 
            ...
            # If still not found, try NA as final fallback (universal guides)
            if not size_guide_result:
                size_guide_result = find_size_guide(cursor, brand_id, category_id, gender, 'NA', subcategory_id)
...
def get_sizes():
    ...
    # First try subcategory-specific guide, then fall back to category-level guide
    size_guide_result = None
    if subcategory_id:
        cursor.execute("""
            SELECT id FROM size_guides 
            WHERE brand_id = %s AND category_id = %s AND gender = %s 
